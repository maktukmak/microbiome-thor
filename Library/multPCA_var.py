...
from utils import logdet, gauss_entropy, multi_loglik_bounded, gauss_loglik
import time


'''
Full Posterior over vectorized W
...
class multPCA_var():
    ...
    def __init__(self, M, K = 5, D = 5, full_dim = False, lmd = 1):
    ...
    def elbo_multi(self, x, y = None):
        ...
        self.ll_obs += - 0.5 * (Ni * np.einsum('j,ijk, k->i', self.W.flatten(), np.kron(self.A, self.S_z), self.W.flatten())).sum()
        self.ll_obs += - 0.5 * (Ni * np.trace((np.kron(self.A, (mmT + self.S_z)) @ self.Gam), axis1=1, axis2=2)).sum()
        
        self.ll_pr = gauss_loglik(self.W.flatten()[None], np.zeros(M * self.K), self.lmd * np.eye(M * self.K))
        self.ll_pr += gauss_entropy(self.Gam)
    ...
    def e_step_inf(self, x, y = None):
        ...
        Gam_p = np.vstack([np.hstack([self.Gam[j * self.K + i].reshape((M, self.K)) for i in range(self.K)]) for j in range(M)])
        G  = (A.flatten() @ Gam_p).reshape((self.K, self.K))
        
        self.Inf_S_out = Ni[None][None].T * np.tile(self.W.T @ A @ self.W + G, (x.shape[0],1,1)) 
    ...
    def m_step(self, x, y = None):
        ...
        self.Gam = np.linalg.inv(self.lmd * np.eye(self.K * M) + np.kron(self.A, (Ni * self.mu_z).T @ self.mu_z + np.sum(np.expand_dims(Ni,-1) * self.S_z,axis = 0)))
        self.W = (sum(np.einsum('id,ik->idk',(xi  + Ni * b), self.mu_z).reshape((-1,M*self.K))) @ self.Gam).reshape((M, self.K))
...
if __name__ == "__main__":
    ...
    M = 100
    C_max = 65000 # Total number of counts - 1 (if 0 -> Categorical)
    full_dim = True
    ...
    model = multPCA_var(M = M, K = K, D = D, full_dim=full_dim, lmd = 1)
    model.fit(X, y, epoch = 2000000, step_comp_elbo=1)
