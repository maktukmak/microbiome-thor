...
class moe():
    def __init__(self, K, Dx, Dt, M,  sk = 1, ss = 1, obs = None):
        ...
        # Init components
        self.gate = glm(M = K-1, D = Dt, obs = 'cat', reg = ss)
        if self.expert_type == 'glm':
            self.experts = [glm(M = M, D = Dx, obs = obs, reg = sk) for k in range(K)]
        elif self.expert_type == 'sklearn':
            if obs == 'gauss':
                self.experts = [Ridge(alpha = sk) for k in range(K)]
                x = np.random.normal(size=100 * Dx).reshape((100, Dx))
                y = np.random.normal(size=100 * M).reshape((100, M))
                [self.experts[k].fit(x, y) for k in range(self.K)]
            elif obs == 'cat':
                self.experts = [LogisticRegression(C = sk) for k in range(K)]
                x = np.random.normal(size=100 * Dx).reshape((100, Dx))
                y = np.random.choice(M+1, size=100)
                [self.experts[k].fit(x, y) for k in range(self.K)]
...
if __name__ == "__main__":
    ...
    if False: # Regression test    
        ...
        start = time.time()
        cnv_vec, q_vec = model_moe.fit(x, x, y)
        end = time.time()
        print('Fit time:', end- start)
        
        yp = model_moe.predict(x, x)[0]
